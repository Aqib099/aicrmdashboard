...
const pickSome = (arr, n) =>  // Fixed: Standardized to pickSome
    [...arr].sort(() => Math.random() - 0.5).slice(0, n);
const weighted = (paris) => {
    const total = paris.reduce((s, [, w]) => s + w, 0);
    let r = Math.random() * total;
    for (const [v, w] of paris){
        if((r -= w) <= 0) return v; 
    }
    return paris[0][0]
};
...
const COMPANIES = [["Acme Corp", "acmecorp.com"], ["Globex", "globex.io"], ["Initech", "initech.com"],
...
["Aperture Labs", "aperture.io"], ["Black Mesa", "blackmesa.org"], // Fixed: blacklesa -> blackmesa
...
["Cogswell Cogs", "cogswell.io"], ["Stark Labs", "starklabs.io"], // Fixed: comma -> dot
["Wayne Tech", "waynetech.io"]]; // Fixed: removed typo ", 10"

const FIRST = [
    "Olivia", "Noah", "Emma", "Liam", "Ava", "Ethan", "Sophia", "Mason", "Isabella",
    "Lucas", "Mia", "Logan", "Amelia", "Jackson", "Harper", "Aiden", "Evelyn", "Caleb", "Abigail",
    "Henry", "Ella", "Owen", "Scarlett", "Wyatt", "Grace", "Julian", "Chloe", "Leo", "Nora", "Eli",
    "Zoe", "Adrian", "Lily", "Miles", "Hannah", "Aria", "Felix", "Ruby", "Hugo", "Maya",
];
...
const TITLES = [
    "VP of Sales", "CTO", "CEO", "Founder", "Head of Operations", "Product Lead", "Procurement Manager",
    "Marketing Director", "Engineering Manager", "COO", "CFO", "Head of Growth", "Account Executive",
    "Solutions Architect", "Director of IT",
];

const TAGS = [
    "decision-maker", "champion", "technical", "finance", "executive",
    "warm", "vip", "influencer", "saas", "enterprise",
];

const SOURCES = ["Website", "Referral", "Cold Outreach", "Social", "Event", "Other"];

const NOTE_TEMPLATES = [
(c) => `Discovery call with ${c} went well - strong interest in the analytics module. Loop in a solutions engineer for the technical deep-dive.`, // Fixed: $(c) -> ${c}
...
const emailFor = (name, domain) => // Fixed: emailfor -> emailFor
   `${slug(name.split(" ")[0])}@${domain}`;

const run = async () => {
    await connectDB();

    let user = await User.findOne({ email: "manqushofficial@gmail.com" });
    ...
    if (user) {
       await Promise.all([
        ...
        user = await User.create({
            name: "Alex Carter",
            email: "manqushofficial@gmail.com",
            password: "Test@1234",
            company: "Manqush",
        });
    } 
    ...
    const leadDocs = [];
    const usedCompanies = pickSome(COMPANIES, 40); // Fixed: PickSome -> pickSome

    for (let i = 0; i < 40; i++) {
        const [company, domain] = usedCompanies[i] || pick(COMPANIES); // Fixed: companies -> company
        const name = personName(); // Fixed: presonName -> personName
        const status = weighted([
           ["New", 28], ["Qualified", 24], ["Proposal", 20], ["Won", 16], ["Lost", 12] 
        ]);
        const ageDays = status === "Won" || status === "Lost" ? rand(20, 175) : rand(0, 120); // Fixed: Status -> status

        leadDocs.push({
            owner,
            name,
            email: emailFor(name, domain), // Fixed: emailFor
            ...
            notes: pick([
                "Inbound from demo website requests.",
    ...
    for (let i = 0; i < 28; i++){
        ...
        taskDocs.push({
            ...
            description: Math.random() < 0.5
                ? pick([
                    "Reference the latest proposal writing.",
                    "Coordinate with the solution engineering team.", 
                    "Confirm the next step and decision timeline."
                ]) : "",
            dueDate,
            status,
            priority: weighted([["High", 35], ["Medium", 45], ["Low", 20]]),
    ...
    console.log("   -> manqushofficial@gmail.com / Test@1234");
